...
- **Where M2 stands:** every WP has landed and is committed; HEAD 0aa8ca2+ is green (535 tests, sim 72/72, 849 KB). In flight: the zoom's code review (2.1), the **M2 milestone review** (a reviewer plays the judge's first 8 minutes), and a small backdrop follow-up (the resting world wyrm's horns behind big wyverns). Next: fix the milestone review's blockers, the lead's own real-time play-through, then the ★ hand-off to the user.
...
| 2.1B Zoom integration and polish: core stages, crowd rally/fuse, Mountain backdrop and world wyrm, beats for audio | builder-max (same agent if alive) | as 2.1A | 2.0, 2.3, 2.9 | ✅ committed 0aa8ca2; review: accept + perf/feel fixes (batched with the milestone review's items) |
...
| 2.9 Crowd M2: lancers (cavalry), champions (Aldric, Brunhild), ability reactions, `rally`/`setFused` | builder-high | `src/render/crowd/**` except `banner.ts` | 2.0, 2.4 | ✅ accepted: 066e378 + fixes ed1c16d (`prepareTier`) |
