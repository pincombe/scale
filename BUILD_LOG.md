...
4. **Size (the strategic risk for M3/M4).** Measured 2026-09-23 on the M2 working tree (source-map attribution, scratch script `attr.mjs`): **794 KB raw = JS 643 KB + fonts 100 KB + CSS 50 KB** (minified by Vite 8's default Oxc; the CSS is minified too). JS by folder: dragon 101, backdrop 92, crowd 62, audio 57, core 51, ui 50, zoom 47, music 45, fx 39, heraldry 33, other render 21, app 21, break_infinity 16, lib 5 KB. Largest files: `dragon/rig.ts` 27 KB, `text.ts` 20, `dragon/paint.ts` 19, `backdrop/worldWyrm.ts` 17, `sfx.ts` 16, `zoom/cinematic.ts` 15. Dev-only code that ships: 14.5 KB (`app/debug*.ts`, `zoom/standin.ts`, `zoom/debug.ts`); no lab, gallery, measure or sim code ships. **Headroom for M3 + M4 is ~200 KB** if nothing changes, too little for eight tiers of bespoke art code. Plan:
   - M3/M4 tiers must be mostly data (parameter sets on shared generators), not bespoke modules.
   - Drop `zoom/standin.ts` once the real pieces land; consider stripping `?debug` from the final build.
   - **Fallback (ask the user at the M2 hand-off):** ship the JS+CSS deflate-compressed inside `index.html` and decompress on load with the built-in `DecompressionStream('deflate-raw')` (Chrome 103+, Safari 16.4+, Firefox 113+). Estimated raw file ~350 KB (fonts are already compressed). Honors both readings of "under 1 MB". A build-pipeline change plus the size guard; do it in M3 if the raw size passes ~850 KB.
...
| 2.1A Zoom director prototype: snapshot, exponential pull-back, colossus, hide, hand-back; stops with key frames for review | builder-max | `src/render/zoom/**`, `src/ui/tierCard.ts` (+ its CSS), `setCinematic` in `src/ui/mount.ts`/`api.ts` | contract | 🔨 running |
...
| 2.2 Boss and gauge juice: boss arrival (darkened sky, red vignette pulse), tremors by gauge, grand boss death into the zoom, the Meadow eye tied to the gauge + time fallback + `onEyeOpen` | builder-high | `src/render/fx/**`, `src/render/post.ts`, `backdrop/eyeTimeline.ts` (narrow) | 2.0, 2.3 | ✅ done (uncommitted: its eye-schedule API is used by 2.3's uncommitted backdrop, so it commits with 2.3); review in flight |
| 2.3 Mountain backdrop: tier-aware backdrop, alpenglow palette, the range as the wyrm's spine, scaly ground, knee-high clouds, the world wyrm's head (`wyrmPose`), `setTransition`, memory per tier | builder-high | `src/render/backdrop/**` (not `eyeTimeline.ts`), `MOUNTAIN` in `src/render/palette.ts` | contract | 🔨 running |
| 2.4 Heraldry renderer: coat of arms from `state.heraldry`, charges as vector art, banners/shield bake, `createHeraldry` | builder-high | `src/render/heraldry/**`, `src/render/crowd/banner.ts`, the `Heraldry` type in `crowd/api.ts` | contract | ✅ committed 367e36a (lead art check: premium); review done → fix round in flight (same agent) |
| 2.5 UI M2 (all of it, one agent for coherence): gauge that becomes the boss timer, boss bar and arrival card, Zoom button, abilities bar (1–3), Scales counter, the height headline, panel tabs Champions and Heraldry (big coat), M2 coach hints and toasts | builder-high | `src/ui/**` except `tierCard.ts` and `setCinematic` (2.1); `app/input.ts` onAbility | 2.0, 2.4 | 🔨 running |
| 2.6 Generative music engine v1 | builder-max | `src/audio/music/**` (engine.ts only if the music bus needs it: flag it) | contract | 🔨 running |
| 2.7 Sim: tier 0–1 pacing (zooms ~3:30 / ~7:30, casual < 5:00), tune `BALANCE` | builder-high | `src/sim/**`, `BALANCE` | 2.0 | 🔨 running (shares `src/sim` with 2.2's eye/juice edits) |
| 2.8 Wyvern species and bosses: wings as forelegs, craggy plates, glide-in, wing hits + `weakRadius`, `leave`, Elder Newt and Grimmaw dressing, cache keyed by tier | builder-max | `src/render/dragon/**` | contract | 🔨 running |
| 2.9 Crowd M2: lancers (cavalry), champions (Aldric, Brunhild), ability reactions, `rally`/`setFused` | builder-high | `src/render/crowd/**` except `banner.ts` | 2.0, 2.4 | 🔨 running |
| 2.10 SFX v2: boss horn, tremors, the eye's rumble, zoom beats, abilities, lancers, champions, heraldry | builder-high | `src/audio/sfx*.ts`, `src/audio/synth/**` | 2.0, 2.1A beats | 🔨 running |
