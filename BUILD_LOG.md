...
| 2.5 UI M2 (all of it, one agent for coherence): gauge that becomes the boss timer, boss bar and arrival card, Zoom button, abilities bar (1–3), Scales counter, the height headline, panel tabs Champions and Heraldry (big coat), M2 coach hints and toasts | builder-high | `src/ui/**` except `tierCard.ts` and `setCinematic` (2.1); `app/input.ts` onAbility | 2.0, 2.4 | ✅ accepted: 357459b + 4829da5 (fixes, staged landing); Aldric portrait follow-up in flight |
| 2.6 Generative music engine v1 | builder-max | `src/audio/music/**` (engine.ts only if the music bus needs it: flag it) | contract | ✅ committed e04371d; review: accept after fixes → fix round in flight (same agent) |
...
| 2.9 Crowd M2: lancers (cavalry), champions (Aldric, Brunhild), ability reactions, `rally`/`setFused` | builder-high | `src/render/crowd/**` except `banner.ts` | 2.0, 2.4 | ✅ committed 066e378; review in flight |
