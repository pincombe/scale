...
|---|---|---|---|---|
| 2.0 M2 core: tiers, gauge, bosses (+`leave`), zoom stages, Scales, Fusion, heraldry, abilities, champions, lancers, text shapes, save v4, debug ops; sim kept green | builder-high | `src/core/**`; `src/sim/**` only to keep targets green; M2 debug controls in `src/app/debugTools.ts`; ARCHITECTURE §4/§14 wording | contract (done) | ✅ committed 9ffe6c7; review done → fix round in flight (same agent) |
...
| 2.4 Heraldry renderer: coat of arms from `state.heraldry`, charges as vector art, banners/shield bake, `createHeraldry` | builder-high | `src/render/heraldry/**`, `src/render/crowd/banner.ts`, the `Heraldry` type in `crowd/api.ts` | contract | ✅ committed 367e36a (lead art check: premium); review in flight |
