...
## Resume here
- **Milestone:** **M2 The Zoom ★ handed to the user for playtesting (2026-09-23).** Every M2 WP is accepted and committed; the M2 milestone review said "the zoom is a wow" and its blocker is fixed. **Waiting for the user's M2 feedback.** Per KICKOFF rules 7–8: deal with the feedback, then hand off (bring this log up to date, commit, tell the user, stop); M3 starts in a new session.
- **Snapshot at the ★ (HEAD 6eab464 + this log):** 540 tests pass, typecheck clean, `npm run sim` 72/72, `dist/index.html` 855 KB raw / 332 KB gzip (limit 2 MB). Lead's real-time check of the first zoom through the real path (boss kill → fall → auto zoom → landing): 1,021 frames, p50 16.7 ms, p99 17.7 ms, max 17.7 ms, none over 25 ms.
- **Where M2 stands:** done. See "M2 known issues at the ★" below for what the user was told.
...
## M2 known issues at the ★ (as told to the user)
- **Audio is unheard by the builders.** The music and SFX were designed and metered offline; no agent can listen. The user's ears are the test (M1: "the game sounds great").
- **Casual pacing leans on buying.** A weak casual bot (3 clicks/s, never aims, slow to find the Upgrades tab) met the Elder Newt at 7:50 (the sim's casual bot zooms at 4:57). Candidates for M3: a catch-up rule or an affordability nudge.
- **No settings UI yet** (M3.5). Reduce motion/flashes follow the OS `prefers-reduced-motion`.
- **M2 ends at Grimmaw.** After him the Zoom button is locked ("The Kingdom arrives in the next build") and nothing new arrives; the army reads as a thin line at the far pull-back.
- **Browsers:** the builders tested Chromium only for M2; the user tested M1 in several browsers. Safari risks: the heavier canvas load (the zoom's peak ~250 MB of canvases at DPR 2; a 5K display doubles it), the boss's grade passes.
- **Minor:** the fx's one-off engage beat can double one heartbeat per boss fight; the coach "Strike where it glows" reappears in the Mountain only if the player never landed a crit in the Meadow.
...
| 2.1A Zoom director prototype: snapshot, exponential pull-back, colossus, hide, hand-back; stops with key frames for review | builder-max | `src/render/zoom/**`, `src/ui/tierCard.ts` (+ its CSS), `setCinematic` in `src/ui/mount.ts`/`api.ts` | contract | ✅ accepted: da67852 |
| 2.1B Zoom integration and polish: core stages, crowd rally/fuse, Mountain backdrop and world wyrm, beats for audio | builder-max (same agent if alive) | as 2.1A | 2.0, 2.3, 2.9 | ✅ accepted: 0aa8ca2 + 6eab464 (final polish); lead art review passed |
| 2.2 Boss and gauge juice: boss arrival (darkened sky, red vignette pulse), tremors by gauge, grand boss death into the zoom, the Meadow eye tied to the gauge + time fallback + `onEyeOpen` | builder-high | `src/render/fx/**`, `src/render/post.ts`, `backdrop/eyeTimeline.ts` (narrow) | 2.0, 2.3 | ✅ accepted: 2f7a512 + e87f491 (the eye counts from the first strike; whole damage numbers) |
| 2.3 Mountain backdrop: tier-aware backdrop, alpenglow palette, the range as the wyrm's spine, scaly ground, knee-high clouds, the world wyrm's head (`wyrmPose`), `setTransition`, memory per tier | builder-high | `src/render/backdrop/**` (not `eyeTimeline.ts`), `MOUNTAIN` in `src/render/palette.ts` | contract | ✅ accepted: 2f7a512 + 3179021 (wyrm sinks) + 5fb5212 (gradual free) |
...
| 2.5 UI M2 (all of it, one agent for coherence): gauge that becomes the boss timer, boss bar and arrival card, Zoom button, abilities bar (1–3), Scales counter, the height headline, panel tabs Champions and Heraldry (big coat), M2 coach hints and toasts | builder-high | `src/ui/**` except `tierCard.ts` and `setCinematic` (2.1); `app/input.ts` onAbility | 2.0, 2.4 | ✅ accepted: 357459b + 4829da5 + af6c36d + e61eae7 (toasts wait for the landing) |
...
| 2.9 Crowd M2: lancers (cavalry), champions (Aldric, Brunhild), ability reactions, `rally`/`setFused` | builder-high | `src/render/crowd/**` except `banner.ts` | 2.0, 2.4 | ✅ accepted: 066e378 + ed1c16d + 4b1d257 (gradual free) |
...
- 2026-09-23: 2.9 fix round committed (ed1c16d): specials always land (12/12 over 11 kills; the wind-up runs through the entrance; `champion.test.ts`), a resync mid-rally rebuilds the pile, `CrowdView.prepareTier(tier)` inside a shared 3 ms per-frame bake budget (switch frame 3.5 ms; 57.3 → 25.2 MB after the switch), buffers with headroom, LOD hysteresis + LRU over the 61 MB ceiling, Charge! from state, the Volley storm across the whole stage, lancers visible by the hero, Rally sparks on the blows. Split plan for `crowd/index.ts` before M3: `knights.ts` (shared records), `cavalry.ts`, `rally.ts`, `reactions.ts`, `draw.ts`, `index.ts` as wiring. **2.9 accepted.** All M2 WPs are accepted except 2.1B (zoom Part B, running: it must call `crowd.prepareTier(to)` at the rally).
- 2026-09-23: **Zoom Part B committed (0aa8ca2).** Lead frame review at 1440×900: the meadow now fills everything around the boots (2.9); its frame closes into one scale ahead of the toes as dark scales and a cloud bank fill in (3.4–3.8); the hold (4.25–5.15) shows the whole sunset inside a scale ~20% of the stage wide, with a sheen and its warm light on the boots: **the twist lands, and it's the game's iconic image**; the cloud bank sinks, the range appears (6.2), the roar (7.3), the card (9.0); a faint warm scale stays under the hero's feet. Backdrop `prepare(tier)` slices (the backdrop's switch share 71 → 1 ms), the rally sound pre-built (44 → 1.8 ms), the sim reads the cinematic's times, stand-ins deleted. Lead decision: the card keeps its full 2.6 s (play resumes at 10.5 s) and the engaged "Grimmaw summoned" window widens to 6:45–7:25 rather than trimming the card to fit the sim. Full check green (535 tests, sim 72/72, 849 KB).
- 2026-09-23: Launched: the zoom's code review, the M2 milestone review (judge run: engaged and casual, real-time zoom, 1440×900 and 1280×800, console, network, CPU/FPS, memory), and the backdrop follow-up for the resting wyrm's horns.
- 2026-09-23: Backdrop follow-up committed (3179021): the resting world wyrm sinks behind the saddle under big dragons (no more horns reading as spikes); the posed reveal unchanged.
- 2026-09-23: **Zoom code review (2.1): accept, no correctness bugs.** Verified by stepping the real loop: each dispatch once (`switch` 2.20 s, `end` 10.50 s); catch-up at 1.0/4.0/9.0/10.8 s carries on; a lost `zoomBegin` replays in full from the resync; debug aborts at 1.6 and 4.6 s are clean; a second zoom after Grimmaw is refused; `reduceMotion` + `reduceFlashes` fine; every path restores the director, the UI, layers, pose, card, snapshot and buffers; the hand-back within 0.01 m of `heroRest`; the post-landing one-offs are gone (≤ 2.5 ms). Frame times avg/p95/max: rally 4.9/9.8/17.2, fusion 2.4/5.0/5.3, boots 1.7/3.2/3.6, closing 1.1/3.2/3.7, hold 0.6/0.8/1.3, landscape 1.4/1.9/3.0, card 1.1/1.6/3.3; the flash 7.5 ms. Canvas memory 122 MB before → ~230 MB peak (rally/fusion; ~250 MB with the main canvas and load-time textures) → 113 MB after: fine on desktop Safari (≥ 2.1 GB); a 5K display roughly doubles it. To fix (batched with the milestone review): (1) the frame after the switch is 18–21 ms because the crowd frees ~408 Meadow canvases at once (13.5 ms): free gradually (~40/frame) or in the hold (4.25–5.15 s, 0.3 ms frames), same for the Meadow backdrop; (2) the rally's first frame 17.2 ms (then ~9–10 ms for 0.4 s): start `prepare(1)`/`prepareTier(1)` at `bossDefeated` when `zoom.count === 0` (the boss's 3 s fall is spare time); (3) the prepared Mountain leaks after an abort before the switch (121.6 MB; debug-only in M2, save-loading in M3); (4) per-frame option objects at `cinematic.ts` ~956/983; (5) `memMB()` misses the colossus buffer; (6) debug "Zoom now" after Grimmaw wipes progress (guard `isLastTier`). **Feel (agreed by the lead):** at the hold the hide's rows are pale lavender like the cloud bank (they read as clouds), and the meadow's scale is taller than its neighbours with its root showing (a window in the ground): make the neighbours darker and crisper and let the row in front cover more of its root, so it reads as one scale among many. M3 note: the close-up hide, cloud bank and `markScale` are Mountain-only; M3 needs a per-tier "ground at any magnification" hook; `cinematic.ts`/`plate.ts` import `backdrop/hide` directly. Zoom code ~63 KB minified.
- 2026-09-23: **M2 milestone review (judge run at HEAD 13cde74): "the zoom is a wow"**, one cheap blocker. Every design beat landed (engaged: gauge 0:40, Aldric 1:37, Elder Newt 2:58 → beaten 3:09.7, zoom 3:13.7 → play 3:24.2, staged landing clear, lancers 3:56, Volley 4:26, Brunhild 4:40, Grimmaw ~7:23 → beaten ~7:34, the locked Zoom button); music moods fired in order. 535 tests, sim 72/72, 849 KB raw / 338 KB gzip; 0 console errors over ~25 min in 4 tabs; exactly 1 network request; 60 fps everywhere at 1440×900 DPR 2 (CPU avg 1.35–1.74 ms, p99 4.5–9.1 ms; the zoom had 3 frames over 20 ms, worst 22); memory flat after 15 min; 1280×800 clean. **Blocker:** the eye opens at the first strike if a player waits ≥ 2:45 on the title (`state.t` runs from page load) → the eye's rules count from the first strike (sent to 2.2's agent). Fix before ★: decimal damage numbers in the Mountain ("8.09": round below 1000; 2.2's agent); the hide's scales read as lilac pillows and a flat top edge at ~3.5 s reads as a window (zoom agent, with the code review's feel notes); an unlock toast mid-landing (hold toasts until the landing ends; UI agent). Later (M3): casual pacing leans on buying (a weak casual bot met the Elder Newt at 7:50: consider a catch-up rule or an affordability nudge; it found the Upgrades tab only at 2:30); no settings UI yet (the lead made `prefers-reduced-motion` the default for reduceMotion/reduceFlashes, 58d84fd); after Grimmaw the army is a thin line of ants and nothing new arrives (M3 zooms there).
- 2026-09-23: Final M2 fix round in flight, batched by owner in disjoint files: 2.2's agent (the eye's clock, damage-number rounding: `eyeTimeline.ts`, `fx/numbers.ts`), the zoom agent (the hold's feel: darker keeled neighbours, the meadow-scale's root covered, no window edge, contrasty boots after the flash; prepare at `bossDefeated`; per-frame objects; `memMB`; the debug guard: `render/zoom/**`, `backdrop/hide.ts`), the crowd agent (gradual free of the old palette), the backdrop agent (gradual free, the prepared-Mountain leak on abort), the UI agent (hold toasts during the landing).
- 2026-09-23: Milestone fixes committed: the eye counts from the first strike and whole damage numbers (e87f491), toasts wait for the landing (e61eae7), the backdrop frees the old tier gradually and no longer leaks after an aborted zoom (5fb5212), the crowd frees the old palette gradually (4b1d257), the zoom's final polish: a dark, crisp dragon's hide at the hold with the meadow's scale level in its row, a soft closing frame, contrasty boots, the pre-bake during the boss's fall (the rally's first frame 17.2 → 5.2 ms) (6eab464); `prefers-reduced-motion` as the default (58d84fd). Full check green: 540 tests, sim 72/72, 855 KB.
- 2026-09-23: **Lead's final checks:** the first zoom in real time through the real path at a locked 60 fps (1,021 frames, max 17.7 ms: the zoom agent's 40–100 ms landing spike was a hidden-pane artifact); the new hold reviewed at 1440×900: one scale among many dark, rim-lit, snow-notched scales, holding the whole sunset. **M2 ★ handed to the user**; pushed to GitHub (Pages deploys on push).
