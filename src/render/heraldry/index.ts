...
export type { Coat, CoatCharge, CoatRegion, ChargeKind, Tincture, Line } from './coat';
...
export function heraldryFor(coat: Coat): Heraldry {
  return { field: tinctureColor(coat.field), tincture: tinctureColor(coat.principal.tincture), charge: coat.principal.kind, coat };
}
...
export function createHeraldry(scene: Scene): void {
  ...
  const refresh = (): void => {
    ...
    hash = n;
    const coat = coatOf(h);
    if (coat.key === key) return;
    key = coat.key;
    scene.crowd.setHeraldry?.(heraldryFor(coat));
  };
  game.on('purchase', (e) => {
    if (e.kind === 'heraldry') refresh();
  });
  game.on('resync', refresh);
  // ~10 Hz: the first refresh after boot flies the loaded coat; also catches debug edits of state.
  scene.ui.onRefresh(refresh);

  // Debug: grow the coat without the economy (?debug).
