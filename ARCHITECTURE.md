...
```
- Phases: `enter` (1.0 s for a newt ≤ 0.6 m, easing in log size to 1.6 s at ≥ 4 m: `enterDuration(size)`) → `idle` (`idleAfterEnter` = 1.0 s, so every dragon telegraphs an attack early) → `windup` (1.2 s, `attack` tells which) → `breath` (1.5 s) | `swipe` (0.9 s) → `idle` (3–6 s) → `windup`…; a weak-spot strike during `windup` → `stagger` (2 s: ×2 army damage, gold bonus). HP ≤ 0 → `dying` (1.1 s for a newt, easing to 1.6 s at ≥ 4 m: `dyingDuration(size)`; a boss (§14) enters in `BALANCE.boss.enter` 2.6 s and falls in `BALANCE.boss.dying` 3.0 s: `dragonEnterDuration(d)` / `dragonDyingDuration(d)`; render animates both on `phaseT / phaseDur`, so they just play faster) → next dragon (`index + 1`) spawns in `enter` (M2: or the boss, or the first zoom begins: §14). `leave` (M2, `dragonEnterDuration(d)`: the entrance's time, so a boss's `BALANCE.boss.enter` 2.6 s): the dragon retreats off stage right (an escaping boss, or a fighting dragon when a zoom begins); it can't be hit, and nothing lands on it. The first dragon starts in `idle` (`firstIdle` 4 s). Durations live in `BALANCE.phase` (read `PHASE` or `dragon.phaseDur`, never hard-code them).
...
**Tiers.** `TIERS` (`core/content/tiers.ts`): per tier `{ id, species, boss, bossAt /*kills to fill the gauge*/, baseHeight /*display m*/, units, champion }`; `tierOf(t)`, `speciesOf(t)`, `lastTier()`. M2 has two: **0 Meadow** (`newt` · `elderNewt` The Elder Newt · footman, archer · `aldric` · bossAt 30 · 1.8 m) and **1 Mountain** (`wyvern` · `grimmaw` Grimmaw of the Peaks · lancer · `brunhild` · bossAt 31 · 200 m). Zooming past the last tier is refused (`sel.isLastTier`; in M2 the Zoom button after Grimmaw says the Kingdom comes in the next build: `MICROCOPY.zoomLocked`). World units stay tier-local meters: a knight is always `KNIGHT_HEIGHT` = `KNIGHT_M` = 1.8 world m, and `state.height` is its **display** height (1.8 in the Meadow; the colossus's height after a zoom: the headline number). A length's display value is `m × state.height / 1.8` (`sel.displayMeters(s, worldM)`).
- **Tier scaling** (`BALANCE.tiers[t]`, past the table each number extrapolates geometrically): HP × `hpMult` (Mountain ×100), kill gold × `goldMult` (×60), unit/upgrade/champion-level costs × `costMult` (×45; `tierHpMult/tierGoldMult/tierCostMult(t)` in formulas), the boss's HP × `bossHp` (below). What you buy in the Mountain is ~1.7× weaker against its dragons than in the Meadow; the upgrades you keep, the Fusion Bonus, heraldry, abilities and champions make up for it. **Dragon #0 of every tier after the Meadow** (the colossus's first opponent) has `BALANCE.dragon.arrivalHp` (0.6) of its curve HP, so it falls a few seconds after the zoom (its gold stays on the curve).
- **Pacing (WP 2.7 sim, 20 seeds, juiced, median (worst); `npm run sim` checks 72 targets):** engaged: Elder Newt summoned 3:06, fight (summon → kill, entrance included) 8.6 s, first zoom 3:19; the first wyvern falls 2.6 s after the zoom ends, the army is back to 20 units 51 s later; lancers 4:10, Volley 4:38, Brunhild 4:51; Grimmaw summoned 7:14, a 12.2 s fight, beaten 7:27 (7:48); damage: clicks 47% of the Meadow's, 52% of the Mountain's, champions 6% / 15%. Casual: first zoom 4:57 (5:17), Grimmaw beaten 11:59 (12:31), champions 26% of the Mountain's damage. NonAimer: Grimmaw beaten 11:22 (11:29). Idle: Elder Newt 10:38, Grimmaw ~28:00. No escapes (idle: one at Grimmaw); the longest gap between kills to Grimmaw's fall: engaged 18.7 s, casual 28.5 s, nonAimer 28.4 s, idle 57 s.
...
**Wyrm Gauge and bosses.** Each ordinary kill adds 1 to `wyrm.charge` (capped at bossAt; not after the boss is beaten); `sel.gauge(s)` = charge / bossAt, 0..1 (1 once cleared). When the gauge is full, the next dragon to spawn (when the current one's `dying` ends) is the boss (`bossSummon {boss, dur}`, then its `dragonSpawn`/`dragonPhase enter`; `dragon.boss` = the boss id). A boss is a dragon in every respect (all phases, weak spots, staggers) but bigger (size × 1.5 of its index), tougher (HP = dragon #bossAt's × the tier's `bossHp`: Elder Newt ×1.4, Grimmaw ×1.3 of #31; **fixed per tier**: an escaped boss comes back no tougher, so the grown army wins) and richer (gold × 4), grander (it enters in `BALANCE.boss.enter` 2.6 s and falls in `BALANCE.boss.dying` 3.0 s, vs a dragon's ~1.6 s; the first zoom begins when the fall ends), and named from `BOSS_TEXT`. Its timer `wyrm.bossT` counts down from `wyrm.bossDur` (30 s) only while it can be hit (not during `enter`, `dying`, `leave` or a hold). No weak spot is live while it (or any dragon) `leave`s (`weakSpotLive`). Selectors: `sel.gauge`, `sel.bossTimeLeft` (0 when no boss fights), `sel.bossClockRunning`, `sel.bossAt(tier)`, `sel.tierBoss`.
- Timer out: `bossEscaped {boss}`; the boss plays `leave` (`dragonEnterDuration`: `BALANCE.boss.enter` 2.6 s, its entrance in reverse, long enough to walk or fly off with dignity; it can't be hit); `wyrm.escapes + 1`; the gauge drops back to ⌊0.75 × bossAt⌋; after the leave the refill **replays the dragons leading up to the boss** (index `bossIndex − (bossAt − charge)` upward), so the boss returns at the same index after the refill and a retry is never harder than the first approach (the army has grown meanwhile). No dead end. Only after the boss is **beaten** do ordinary dragons continue past it (`index + 1`, still growing: the push-further choice).
...
3. `end`: `zoom.stage = null`, `pending = null`; play resumes and dragon #0 makes its entrance. Emits `zoomEnd {tier}`, then `dragonPhase {id, phase: 'enter', dur}` for dragon #0 (its entrance starts now: one-shot entrance effects such as SFX and the wyvern's landing listen for it).
**What fires when:** `begin` → `zoomBegin`, then the fighting dragon's `dragonPhase leave`. `switch` → `scalesGain`, `zoomSwitch`, `resync` (dragon #0 is already in state, frozen at the start of `enter`: no `dragonSpawn`), then the new tier's `unlock`s. `end` → `zoomEnd`, then `dragonPhase enter`.
**Reconcile from state (every consumer):** events can be lost (`Game.catchUp` drops them while the tab is hidden, e.g. the first boss dying in a background tab drops its automatic `zoomBegin`). On `resync` (and on load), read `state.zoom`: stage `'begin'` with no cinematic running → the zoom director starts it from `zoom.pending`; stage `'switched'` → it dispatches `end`; UI, music and audio likewise derive "a zoom is holding" from `zoom.stage`, never only from the events.
...
**Scales and the Fusion Bonus.** `sel.scalesForZoom(s)` = ⌊the tier's `scales` (Meadow 6, Mountain 24) × 1.15^(kills since the boss fell)⌋: the first zoom pays 6, three first-level charges; `sel.fusionForZoom(s)` = 1 + 0.1 × √(units fused) × (1 + 0.5 × Crown level), rounded to 0.01 (an engaged player's ~120 units at the Elder Newt: ×2.1). `sel.heightForZoom(s)` = the next tier's `baseHeight` × fusion^0.15, to 3 significant digits (×2.1 → 224 m). `sel.zoomPreview(s)` → `{scales, fusion, height, units, push}` for the Zoom button. `zoom.fusion` multiplies all damage (clicks, army, champions, abilities) for the rest of the loop.

**Heraldry** (`buyHeraldry {id}`, costs Scales: `sel.heraldryCost(s, id)` = ⌈2 × 1.8^level⌉: 2, 4, 7, 12, 21…): `heraldry.levels[id]` + 1; the first purchase of a charge appends it to `heraldry.order` (order[0] is the principal charge on the shield). Effects per level (data: `CHARGES[id].effect` / `BALANCE.heraldry`, kinds `damageMult|goldMult|weakMult|cooldownMult|startUnits|fusionBonus`, the UI writes the line from it): **lion** all damage ×1.5, **sun** gold ×1.5, **wyvern** weak-spot crit ×1.25, **stag** ability cooldowns ×0.85 (floor 0.4), **tower** +5 footmen per level at once when bought (with the usual `milestone` events if a threshold is crossed; no unit `purchase` event: the crowd reconciles from `state.units`) and +5 per level at the start of every tier (the switch), **crown** Fusion Bonus growth +50% (forward-looking: it shows at the next zoom, so the UI labels it "next zoom"). Unlocked by the first zoom (`feature.heraldry`). Emits `purchase {kind: 'heraldry', id, amount: 1}`. Selectors: `heraldryVisible/heraldryAffordable/heraldryLevel`. `CHARGE_IDS` is the panel order. render/heraldry derives the coat of arms from `state.heraldry` alone.

**Abilities** (`useAbility {id}`; flags `ability.<id>`; the Mountain unlocks charge and rally at once, volley at its 10th kill): ready when unlocked, `cooldown === 0` and not holding. The cooldown starts at use and runs alongside the effect. **charge** (Charge!, 6 s, cooldown 45 s): army and champion damage ×3 while active. **rally** (8 s, 75 s): 8 auto-strikes/s at plain click damage (`strike` events with `auto: true`, `aimed: false`, never crits or staggers; not counted in `stats`). **Auto strikes have no impact point** (`x = y = 0`): render picks `dragon.impactPoint()` for them. **volley** (Dragonbane Volley, 90 s; ready only while the dragon can be hit: not dying or leaving, and past `ENTER_WEAK_FROM` of its entrance, so the cooldown is never spent on a corpse; `sel.abilityReady` reflects it so the UI can grey the button for that moment): one huge volley of 10 s × (army + champion DPS), at least 10 s of 5 plain clicks/s, so it hits even with no army (`volley {ability: true, arrows: 60, flight: 1.1}`, then its `armyHit {ability: true}`). Emits `abilityUse {id, dur}` (volley: dur 0), `abilityEnd`, `abilityReady`. Stag heraldry shortens cooldowns. Holds pause everything; the switch resets actives and cooldowns. `AbilityState.cooldownDur` holds the full cooldown the last use started with (0 when ready), so buying Stag mid-cooldown doesn't stall the bar. `ABILITIES[id]` has `name, flavor, key, dur, cooldown, unlock`. Selectors: `abilityVisible`, `abilityReady`, `abilityActive`, `abilityActiveFrac` (effect left, 1..0), `abilityFrac` (cooldown progress 0..1, 1 = ready), `abilityCooldown` (full, with Stag). Keys 1–3 (`input.onAbility`).

**Champions** (`champion.<id>` flags; `levelChampion {id, amount}` costs gold, `sel.championCost(s, id, n)` = baseCost × the tier's costMult × 1.3^(level-1) per level, up to `maxLevel` 15 (mastered: `sel.championMastered`), `BUY_MAX` supported, all-or-nothing): each tier's champion joins by itself at a kill count (`championJoin` then `unlock {kind: 'champion'}`, level 1, free): Ser Aldric at the Meadow's 20th kill (~1:50 engaged), Dame Brunhild at the Mountain's 12th. They persist through zooms (levels kept). **Damage model: a share of the current dragon's max HP** (bosses included), so a champion counts the same in every tier and a blow never one-shots. A blow lands on every footman melee beat (1 s; even with no footmen; not during enter/breath/swipe/dying/leave): `championHit {id, damage}` (damage applied); every `specialEvery` s (Aldric 9, Brunhild 15) the special lands as soon as the dragon can be hit: `championSpecial {id, damage}` (damage applied: it visibly chunks the HP bar). Shares rise from level 1 to mastery (Aldric: blows 0.65% → 1%, specials 3.25% → 5%; Brunhild: blows 0.05% → 0.15%, rare big specials 2.5% → 4%), × stagger ×2 and Charge! ×3 when they land; the Fusion Bonus and Lion don't apply. Engaged sim: champions do ~6% of the damage in the Meadow after Aldric joins and ~15% in the Mountain up to Grimmaw (casual ~11% / ~26%: they carry weaker players, which is what gets a non-aimer through Grimmaw inside its timer). Selectors: `championVisible`, `championAffordable`, `championMastered`, `championLevelsLeft`, `championCost`, `championMaxAffordable`, `championBlowShare`, `championSpecialShare`, `championHit`, `championSpecialDamage`, `championDps`, `championsDps` (against the dragon on stage). `CHAMPIONS[id]` has `name, title, special, unlock`.
...
**The Mountain's upgrades** (tier 1 only, costs in tier-0 gold × 45, persist like the rest): `highForge` click ×1.25 (1st kill) · `pikeWall` footmen ×4 (3rd kill) · `yewLongbows` archers ×4 (5 archers) · `mountainTithe` gold ×2 (8th kill) · `couchedLances` lancers ×3 (3 lancers) · `destriers` lancer period ×0.7 (10 lancers). The click chain stays modest (the Meadow's ×6 came along) and the army's are strong, so the army matters in the Mountain.
