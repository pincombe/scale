...
- **Tier scaling** (`BALANCE.tiers[t]`, past the table each number extrapolates geometrically): HP × `hpMult` (Mountain ×150), kill gold × `goldMult` (×60), unit/upgrade/champion-level costs × `costMult` (×60; `tierHpMult/tierGoldMult/tierCostMult(t)` in formulas). What you buy in the Mountain is 2.5× weaker against its dragons than in the Meadow; the upgrades you keep, the Fusion Bonus, heraldry, abilities and champions make up for it. Sim (20 seeds, median; WP 2.0 fix round): engaged Elder Newt 3:05, first zoom 3:17, Grimmaw summoned 7:13, beaten 7.5 min (worst 7.8); casual Elder Newt 4:39, zoom 4:59, Grimmaw beaten 12.9 min (worst 13.3); nonAimer Grimmaw beaten 12.2 min (worst 12.5); no escapes in 20 seeds.
